<template>
    ...
    <el-dialog v-model="fullImageModel.show" :title="fullImageModel.label" top="5vh" width="80%">
        <div style="height: 75vh; width: 70%;">
            <img v-if="imageExist" :src="fullImageModel.src" style="width: 100%; height: 100%; object-fit: contain;">
            <div id="full-img-error-icon" v-else-if="!imageExist" :style="{height: '73vh', opacity: 0.6}">
                <!-- <div :style="{fontSize: (0.75 * rem) + 'px'}">无印章图片</div> -->
                <img :src="fullImageModel.eventInfo.image_href" style="width: 100%; height: 100%; object-fit: contain;">
            </div>
            <div id="full-img-error-icon" v-else-if="imageExist === 'null'" :style="{height: '73vh', opacity: 0.6}">
                <div :style="{fontSize: (0.75 * rem) + 'px'}">正在加载</div>
            </div>
        </div>
        <div class="full-image-text">
            <div class="fullImage-title">
                <div :style="{fontSize: rem + 'px'}">{{ fullImageModel.eventInfo.library }}</div>
                <div :style="{fontSize: rem + 'px'}">{{ fullImageModel.eventInfo.Time }}</div>
            </div>
            <div class="fullImage-title">
                <div class="content-value" :style="{fontSize: (0.8 * rem) + 'px'}">
                    <svg style="width: 1em; height: 1em; transform: translateY(0.1em);"
                    t="1677920887221" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2810" ><path fill="#8F7B6C"
                    d="M512 88.174592c-178.7904-3.196928-326.373376 139.101184-329.663488 317.889536 0 264.907776 274.70336 529.814528 329.663488 529.814528s329.662464-264.906752 329.662464-529.814528C838.3488 227.275776 690.811904 84.978688 512 88.174592zM512 606.217216c-104.031232 0-188.379136-84.348928-188.379136-188.379136S407.968768 229.458944 512 229.458944s188.379136 84.347904 188.379136 188.378112S616.031232 606.217216 512 606.217216z" p-id="2811"></path></svg>
                    {{ fullImageModel.eventInfo.location_std_new.replace(/\?|？/i, "") }}</div>
                <div class="content-value" :style="{fontSize: (0.8 * rem) + 'px'}">{{ fullImageModel.eventInfo.lib_type }}</div>
            </div>
            <div class="fullImage-title">
                <div class="content-head" :style="{fontSize: (0.8 * rem) + 'px'}">经手人</div>
                <div class="content-value" :style="{fontSize: (0.8 * rem) + 'px'}">{{ fullImageModel.eventInfo.agent }}</div>
            </div>
            <div class="fullImage-title">
                <div class="content-head" :style="{fontSize: (0.8 * rem) + 'px'}">经手人身份</div>
                <div class="content-value" :style="{fontSize: (0.8 * rem) + 'px'}">{{ fullImageModel.eventInfo.agent_type }}</div>
            </div>
            <div class="fullImage-title">
                <div class="content-head" :style="{fontSize: (0.8 * rem) + 'px'}">经手方式</div>
                <div class="content-value" :style="{fontSize: (0.8 * rem) + 'px'}">{{ fullImageModel.eventInfo.經手方式 }}</div>
            </div>
        </div>
    </el-dialog>

    <div class="circle-card-link" :id="bookName+'-card-link'" :style="{top: event_detail.y + 'px', left: 3 * rem + 'px', width: canvas_width * 0.95 + 'px'}">
        <svg id="card-link-svg" :style="{width: card_container_width + 'px'}"></svg>
    </div>

    <div class="event-container-all" :id="bookName+'-event-container-all'" :style="{top:event_detail.y + 45 + 'px', left: 0.2 * canvas_width + 'px', height: (11.7 * rem + 45) + 'px', width: canvas_width * (0.95 - 0.2) + 'px'}">
        ...
        <div class="timeline-event-seq" :style="{width: card_container_width + 'px', left: -0.2 * canvas_width + 'px'}">
            <div class="timeline-event-seq event-sequence" v-for="(item, index) in JSON.parse(JSON.stringify(event_detail_pop))" :key="index" :style="{left: item.card_pos.x_pos - 0.5 * rem + 'px', top: 0 + 'px', backgroundColor: pos_color_list[index].color, width: (1.2 * rem) + 'px', height: (1.2 * rem) + 'px'}">{{item.次序}}</div>
            <div class="timeline-event-seq time-range" v-for="(item, index) in JSON.parse(JSON.stringify(event_detail_pop))" :key="index" :style="{left: (item.card_pos.x_pos + 0.9 * rem) + 'px', top: (-0.1 * rem) + 'px', width: (6 * rem) + 'px', height: (2 * rem) + 'px'}">{{item.time_info.info_card_Time}}</div>
        ...
        <div class="timeline-event-container" :style="{top:5 + 'px', left: - 0.2 * canvas_width + 'px', height: (10.5 * rem) + 'px', width: card_container_width + 'px'}">
            ...
            <div class="timeline-event-container event-info-card" v-for="(item, index) in JSON.parse(JSON.stringify(event_detail_pop))" :key="index" :style="{top: 0, left: item.card_pos.x_pos + 'px', width: (6 * rem) + 'px', height: (10 * rem) + 'px'}">  
                <!-- <img class="hidden-image" :id="'pic-' + (index)" :src="item.image_href" style="max-width: 100%; margin-top: 0.1em; margin-bottom: 0em"> -->
                <div class="info-row library-name" :style="{fontSize: rem + 'px', height: div_height.library_name + 'px'}">{{item.library}}</div>
                <div class="info-row event-loc" :style="{fontSize: (0.75 * rem) + 'px', height: div_height.event_loc + 'px'}">
                    <svg style="width: 1em; height: 1em; transform: translateY(0.1em);"
                ...
                <div class="info-row agent-action" id="img-above-div" :style="{height: div_height.agent_action + 'px'}">
                ...
                :style="{top: 0.2 * rem + 'px'}"
                v-if="imgDiv.visible" @click="fullImageModel.show = hiddenImgIndexList.includes(index) ? false : true, fullImageModel.src=item.full_image_href, fullImageModel.label= item.image_label, fullImageModel.eventInfo=item">
                    ...
                    <div id="img-error-icon" v-if="hiddenImgIndexList.includes(index)" :style="{height: div_height.pic_height + 'px', width: '98%', opacity: 0.6}"> <!--5 * rem-->
...
export default {
    ...
    computed: {
        ...mapState(["filter", "selection", "hover", "transition", "rem", "language", "switchWholeView", "imageSource"])
    },
    watch:{
        ...
        fullImageModel: {
            handler: function(newVal, oldVal) {
                const self = this
                console.log('fullImageModel', newVal)
                if (newVal.show) { // 若此时需要显示Full Image
                    // console.log('要显示大号图片啦')
                    let full_image = new Image()
                    full_image.src = self.fullImageModel.src
                ...
                } else { // 若此时不显示Full Image
                    self.imageExist = 'null'
                }
            },
            deep: true
        }     
    ...
    methods: {
        initialize() {
            ...
            const self = this
            if (['尚書正義', '論語注疏'].includes(self.event_detail.book_name)) {
                // 使用数组解构和临时变量来交换元素
                let temp = self.event_detail_pop[0]
                self.event_detail_pop[0] = self.event_detail_pop[1]
                self.event_detail_pop[1] = temp

                // 更换顺序名称
                self.event_detail_pop[0].次序 = 1
                self.event_detail_pop[1].次序 = 2
             } else if (self.event_detail.book_name === '文選（明州本）') {
                let temp = self.event_detail_pop[1]
                self.event_detail_pop[1] = self.event_detail_pop[2]
                self.event_detail_pop[2] = temp

                // 更换顺序名称
                self.event_detail_pop[1].次序 = 2
                self.event_detail_pop[2].次序 = 3
             }
            // 利用clip-path制作mask
        ...
        compute_dist() {
            ...
            for (let index in self.event_detail_pop) { // 遍历每一个事件
                 // 用于修改语言
                 if (self.language === 'zh') {
                    if (self.event_detail_pop[index].library === '江戶醫學館(初名“躋壽館”)') {
                        console.log('江戶醫學館(初名“躋壽館”)')
                        self.event_detail_pop[index].library = '江戶醫學館'
                    } else {
                ...
                for (let i of self.event_detail_pop[index].來源.matchAll(/\([^\(\)]+\)\<[^\(\)]+\>/g)) {
                    if (i[0]) {
                        ...
                        self.event_detail_pop[index]['image_label'] = i[0].split('<')[1].split('.')[0].split('/')[2]
                        self.event_detail_pop[index]['image_href'] = (self.imageSource  === 'offline' ? "./assets/images/" : 'http://vis.pku.edu.cn/chinese_classics_circulation/assets/images/') + i[0].split('<')[1].split('.')[0] + '.webp'
                        ...
                        self.event_detail_pop[index]['full_image_href'] = (self.imageSource  === 'offline' ? "./assets/images_full/" : 'http://vis.pku.edu.cn/chinese_classics_circulation/assets/images_full/') + i[0].split('<')[1].split('.')[0] + 'F.jpg'
                    } else {
                        // self.event_detail_pop[index]['image_label'] = '--'
                        self.event_detail_pop[index]['image_label'] = 'none'
                        self.event_detail_pop[index]['image_href'] = (self.imageSource  === 'offline' ? "./assets/images/" : 'http://vis.pku.edu.cn/chinese_classics_circulation/assets/images/') + 'none' + '.webp'
                        self.event_detail_pop[index]['full_image_href'] = (self.imageSource  === 'offline' ? "./assets/images_full/" : 'http://vis.pku.edu.cn/chinese_classics_circulation/assets/images_full/') + 'none' + 'F.jpg'
                ...
                // 判断div的高度
                if ((self.event_detail_pop[index].library).length > 6) { // library_name
                    self.div_height.library_name = 2.1 * self.rem
                }
                const library_name_len = (Math.ceil((self.event_detail_pop[index].library).length / 6) + 0.1) * self.rem * (1 + 0.2),
                      cur_event_loc_len = Math.ceil((self.event_detail_pop[index].location_std_new.replace(/\?|？/i, "")).length / 7) * (0.75 + 0.1) * self.rem,
                      agent_action_len = Math.ceil(((self.event_detail_pop[index].agent).length + (self.event_detail_pop[index].經手方式).length) / 7) * (0.75 + 0.1) * self.rem
                self.div_height.library_name = Math.max(self.div_height.library_name, library_name_len)
                self.div_height.event_loc = Math.max(self.div_height.event_loc, cur_event_loc_len)
                self.div_height.agent_action = Math.max(self.div_height.agent_action, agent_action_len)           
            }
            self.div_height.pic_height = 9.8 * self.rem - (self.div_height.library_name + self.div_height.event_loc + self.div_height.agent_action)
            ...
            for (let index in self.event_detail_pop) {
                ...
                // const img_div_height = 6.2 * self.rem - (library_row_height + location_row_height + location_agent_height)
                const img_div_height = self.div_height.pic_height
        ...
        add_event_card_pos() {
            ...
            self.card_container_width = card_pos_list[card_pos_list.length - 1].x_pos + 8.3 * self.rem
            ...
            if (self.event_detail.book_name === '通典') {
                self.event_detail_pop[0].card_pos.x_pos += 5
             }
        }
    },
    mounted() {
