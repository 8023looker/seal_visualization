...
data() {
        ...
        events: {
            handler: function(newVal, oldVal) {
                ...
                if (newVal !== null) {
                    ...
                    //     console.log(self.timeScale(self.events[i].time_info.timestamp))
                    // }
                    // RenderAxis.draw_axis_circle(self.timeScale, self.events, self.event_offset, self.rem) // 不绘制circle
                }
            },
            deep: true
        }
    ...
    methods: {
        ...
        // sHeight= document.getElementsByClassName('main-panel')[0].clientHeight / 8.36, // 无法实时渲染
        ...
        const width = sWidth;
        // const width = $(".main-panel").width() * 1.176
        ...
        g.append("rect")
            .attr("x", 0)
            .attr("y", 3 * divloc - divloc / 6) // 3 * divloc / 10 + 4 * divloc
            .attr("width", divloc / 4)
            .attr("height", divloc / 4)
            .attr("stroke", "#6e4d2b")
            .attr("fill", "#6e4d2b");
        g.append("text")
            .attr("x", divloc / 2)
            .attr("y", 3 * divloc + divloc / 6) // 4 * divloc + 6.5 * divloc / 10
            .style("fill", "#6e4d2b")
            .attr("font-size", 18)
            .attr("font-family", "FZQINGKBYSJF")
            // .text("日本朝代")
        ...
        this.container
            .append("g")
            .attr("transform", `translate(${0}, ${3 * divloc - divloc / 3 + gap + th})`)
            .call(bottomAxis)
            .call((g) => {
            g.select(".domain")
                .attr("stroke", "#6a4c2a")
        ...
        this.container
            ...
            .call((g) => {
                g.append("line")
                .attr("transform", `translate(${0}, ${divloc / 2 - 0.5 * gap})`) // divloc + divloc / 5 + 0.5 * gap
                .attr("stroke", "#ad9278")
                .attr("y2", th + divloc / 5)
                .attr("stroke-width", 0.8)
                .style('display', (d) => d[0] === '宋'? 'none': 'block')
                g.append("text")
                .attr("transform", `translate(${3}, ${divloc / 2 - 0.5 * gap + th * 3 / 4})`)
                .text(d => {
                    if (this.language === 'zh') {
        ...
        this.container
            ...
            .call((g) => {
                ...
                g.append("text")
                    ...
                    .attr('id', (d) => `${d[0]}-text`)
                    .attr("transform", `translate(${3}, ${divloc + divloc / 5 + 0.5 * gap + th * 3 / 4})`)
                // .selectAll("tspan")
                // .data((d) => d[0])
                // .join("tspan")
                // .text((d) => d[0])
        ...
        this.container // 目前是最上面的轴
            .append("line")
            .attr("transform", `translate(${0}, ${1.5})`) // 移动到最上面
            .attr("x1", 0.08 * sWidth)
            .attr("x2", iw)
            .attr("stroke", "#6a4c2a")
            .attr("stroke-width", 2);
